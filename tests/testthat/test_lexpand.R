...
testthat::test_that("original total pyrs equals pyrs after splitting w/ large number of breaks", {
  ...
  x[,
    fot := get.yrs(ex_date, year.length = "actual") -
      get.yrs(dg_date, year.length = "actual")
  ]
  totpyrs <- x[, sum(fot)]
...
testthat::test_that("lexpanding with aggre.type = 'unique' works", {
  ...
  setDT(ag1)
  ag1 <- ag1[,
    list(pyrs = sum(lex.dur), from0to1 = sum(lex.Xst == 1L)),
    keyby = list(
      fot = popEpi:::cutLow(fot, BL$fot),
...
testthat::test_that("lexpanding with aggre.type = 'cartesian' works; no time scales used", {
  ...
  e <- quote(list(
    sex = factor(sex, 0:1, c("m", "f")),
    period = cut(
      get.yrs(dg_date),
      get.yrs(as.Date(paste0(seq(1970, 2015, 5), "-01-01")))
    )
  ))
  ag1[, c("sex", "period") := eval(e)]
  ceejay <- do.call(
    CJ,
    lapply(ag1[, list(sex, period)], function(x) {
      if (is.factor(x)) levels(x) else unique(x)
    })
  )
  setkey(ceejay, sex, period)
  setkey(ag1, sex, period)
  ag1 <- ag1[
    ceejay,
    list(pyrs = sum(lex.dur), from0to1 = sum(lex.Xst == 1L)),
    by = .EACHI
...
testthat::test_that("lexpand aggre produces correct results", {
  ...
  x <- lexpand(
    ...
    breaks = list(
      fot = c(0, 5, 10, 50, Inf),
      age = c(seq(0, 85, 5), Inf),
      per = 1993:2013
    ),
    status = status,
    aggre = list(fot, age, per)
  )
  setDT(x)
  row_length <- x[, list(
    length(unique(age)),
...
testthat::test_that('lexpand aggre: multistate column names correct', {
  ...
  testthat::expect_equal(
    intersect(names(x), c('from0to0', 'from0to1', 'from0to2')),
...
testthat::test_that('lexpansion w/ overlapping = TRUE/FALSE produces double/undoubled pyrs', {
  ...
  setDT(x)
  testthat::expect_equal(
    x[, sum(lex.dur), keyby = lex.id]$V1,
    sire2[, sum(ex_yrs - bi_yrs), keyby = id]$V1
  )
...
testthat::test_that("different specifications of time vars work with event defined and overlapping=FALSE", {
  ...
  testthat::expect_error(
    ...
    regexp = paste0(
      "some rows have simultaneous 'entry' and 'event', ",
  ...
  testthat::expect_error(
    ...
    regexp = paste0(
      "subject\\(s\\) defined by lex.id had several rows ",
