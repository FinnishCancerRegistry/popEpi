...
set.seed(5)
p18 <- data.table(
  OBS = round(runif(36) * 10),
  PYRS = round(runif(36) * 10000),
  AGEGROUP = 1:18,
  COV = rep(c(1, 2), each = 18)
)
set.seed(5)
p20 <- data.table(
  OBS = round(runif(20) * 10),
  PYRS = round(runif(20) * 10000),
  AGEGROUP = 1:20,
  COV = rep(c(1, 2), each = 20)
)
set.seed(5)
p101 <- data.table(
  OBS = round(runif(101) * 10),
  PYRS = round(runif(101) * 10000),
  AGEGROUP = 1:101,
  COV = rep(c(1, 2), each = 101)
)
p18b <- data.table(p18)
...
testthat::test_that("rate works with different weights", {
  ...
  testthat::expect_equal(
    w4$rate,
    p20[, list(sum(OBS) / sum(PYRS)), by = 'COV'][, V1]
  )
...
testthat::test_that("rate CIs and SEs are correct", {
  ...
  a2 <- ci[,
    sqrt(sum(std.pop^2 * ((obs / pyrs) * (1 - obs / pyrs)) / pyrs)) /
      sum(std.pop)
  ]
  ...
  ci0 <- rate(
    ...
    print = NULL,
    adjust = 'agegroup',
    weights = list(agegroup = c(2773, 2556, 1113, 184))
  )
  ...
  testthat::expect_equal(
    c(ci0$rate.adj.lo, ci0$rate.adj.hi),
    c(a1 - a2 * 1.96, a1 + a2 * 1.96),
    tolerance = 0.0006
  )
...
testthat::test_that("makeWeightsDT works in rate", {
  ...
  mw1 <- makeWeightsDT(
    p18,
    adjust = substitute(factor(AGEGROUP, 1:18, 1:18)),
    weights = wv,
    print = NULL,
    values = list(op)
  )
  ...
  testthat::expect_equal(
    c(attlist$adVars, attlist$vaVars, 'weights'),
    c('factor', 'OBS', 'PYRS', 'weights')
  )
...
testthat::test_that("rate works with different weights an subset", {
  ...
  testthat::expect_equal(
    s4$rate,
    p20[AGEGROUP != 1, list(sum(OBS) / sum(PYRS)), by = 'COV'][, V1]
  )
  testthat::expect_equal(
    s5$rate,
    p20[COV == 1, list(sum(OBS) / sum(PYRS))][, V1]
  )
...
testthat::test_that("rate works with different weights and syntaxies", {
  ...
  s1 <- rate(
    ...
    obs = 'OBS',
    pyrs = 'PYRS',
    adjust = factor(AGEGROUP, 1:18, 1:18),
    weights = c(
      .1,
      .1,
      .1,
  ...
  testthat::expect_error(
    rate(
      ...
      obs = 'OBS',
      pyrs = 'PYRS',
      adjust = list(factor(AGEGROUP, 1:18, 1:18), COV),
      weights = list(wv, c(0.5, 0.5))
    )
  ) # a list length of 2
  testthat::expect_error(
    rate(
      ...
      pyrs = 'PYRS',
      print = 'COV',
      adjust = c('AGEGROUP', 'COV'),
      weights = list(COV = c(.5, .5), AGEGROUP = 1:18)
    )
  ) # duplicated names
  ...
  s11 <- rate(
    ...
    pyrs = 'PYRS',
    adjust = list(
      agegr = factor(AGEGROUP, 1:18, 1:18),
      gender = factor(COV, 1:2, 1:2)
    ),
    weights = list(
      agegr = c(
        .1,
  ...
  s14a <- rate(
    ...
    print = NULL,
    adjust = c('AGEGROUP', 'COV'),
    weights = list(AGEGROUP = 1:18, COV = c(.5, .5))
  ) # SAMA1
  s14b <- rate(
    data = p18,
    obs = 'OBS',
...
testthat::test_that("in rate levels of adjust are printable", {
  w1 <- rate(
    ...
    obs = 'OBS',
    pyrs = 'PYRS',
    print = list(COV, AG = findInterval(AGEGROUP, c(0, 8, 20))),
    adjust = 'AGEGROUP',
    weights = 'cohort'
  )
  w2 <- rate(
    ...
    obs = 'OBS',
    pyrs = 'PYRS',
    print = list(COV, ag = findInterval(AGEGROUP, c(1, 8, 20))),
    adjust = 'AGEGROUP',
    weights = 'world_2000_20of5'
  )
...
testthat::test_that("rate_ratio works", {
  ...
  # rate obj, one line
  testthat::expect_equal(
    rate_ratio(w1[1], w1[2], crude = FALSE, SE.method = FALSE),
    rate_ratio(w1[1], w1[2], crude = TRUE, SE.method = FALSE)
  ) # oikein, crude ignoorattu
  testthat::expect_equal(
    rate_ratio(w1[1], w1[2], crude = FALSE, SE.method = TRUE)$rate_ratio,
  ...
  testthat::expect_equal(
    a,
    (c(88, 78) / c(109818, 110421))[1] / (c(88, 78) / c(109818, 110421))[2],
    tolerance = 0.001
  )
