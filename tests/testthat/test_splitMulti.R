...
testthat::test_that("splitMulti and splitLexis are congruent", {
  ...
  for (sc in seq_along(BL)) {
    testthat::test_that(
      paste0(
        "splitLexisDT and splitLexis congruent with multiple rows per id with breaks no. ",
        sc
      ),
      {
        testthat::expect_identical(1L, 1L) ## to trigger testing...
  ...
  x <- Lexis(
    data = sire2[dg_date < ex_date & !duplicated(id)],
  ...
  for (sc in seq_along(BL)) {
    testthat::test_that(
      paste0(
        "splitLexisDT and splitLexis congruent with multiple Lexis states per id using breaks list no. ",
        sc
      ),
      {
        testthat::expect_identical(1L, 1L) ## to trigger testing...
...
testthat::test_that("splitMulti agrees with splitLexis, vol. II", {
  ...
  lex <- Lexis(
    entry = list(age = agein, per = agein + dob),
