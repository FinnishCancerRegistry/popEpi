...
testthat::test_that("misc things work", {
  ms1 <- sir(
    ...
    coh.obs = c('from0to1'),
    coh.pyrs = 'pyrs',
    ref.data = data.table(popEpi::popmort),
    ref.rate = 'haz',
    adjust = c('agegroup', 'year', 'sex'),
    print = list(
      age.cat = cut(agegroup, c(0, 75, 120)),
  ...
  testthat::expect_equal(
    data.table(confint(ms1)),
    data.table('2.5 %' = ms1$sir.lo, '97.5 %' = ms1$sir.hi),
    tolerance = 0.001
  )
})
...
testthat::test_that("confidence intervals an other options works", {
  ...
  testthat::expect_equal(
    ea[, EAR],
    round((1490 - 1482.13) / 39905.92 * 1000, 3)
  )
...
testthat::test_that("SIR works with multistate aggregated lexpand data", {
  ...
  dummy.coh <- data.table(
    agegroup = 1:18,
    sex = 1,
    from0to1 = round(runif(18)),
    from0to2 = round(runif(18) + 0.4),
    pyrs = (rnorm(18) + 100) * 200
  )
  dummy.ref <- data.table(
    agegroup = rep(1:18, times = 2),
    sex = 1,
    obs = floor(runif(36) * 100),
    categ2 = rep(c(1, 2), each = 18),
    pyrs = (rnorm(36) + 100) * 2000
  )
  ...
  sm <- sir(
    ...
    ref.pyrs = 'pyrs',
    mstate = 'categ2',
    adjust = c('agegroup', 'sex', 'categ2'),
    print = c('categ2')
  )

  setkey(12345)
  dummy.coh2 <- data.table(
    agegroup = rep(1:18, times = 6),
    sex = rep(1:2, each = 18),
    obs = floor(runif(18 * 6) * 100),
    level = rep(1:6, each = 18),
    pyrs = (rnorm(36) + 100) * 2000
  )
...
if (requireNamespace("splines")) {
  testthat::test_that("SIR spline throws errors correctly", {
    ...
    sp0 <- suppressWarnings(try(sirspline(
      coh.data = c,
      coh.obs = 'from0to2',
      coh.pyrs = 'pyrs',
    ...
    sp1 <- suppressWarnings(try(sirspline(
      coh.data = c,
      coh.obs = c('from0to1', 'from0to2'),
      coh.pyrs = 'pyrs',
      subset = year %in% 1990:2008,
    ...
    sp2 <- suppressWarnings(try(sirspline(
      coh.data = c,
      coh.obs = c('from0to1', 'from0to2'),
      coh.pyrs = 'pyrs',
      subset = year %in% 1990:2008,
      ref.data = data.table(popEpi::popmort),
      ref.rate = 'haz',
      dependent.spline = FALSE,
      adjust = c('agegroup', 'year', 'sex'),
      print = c('cause'),
      mstate = 'cause',
      spline = c('agegroup', 'year', 'fot')
    )))
    sp3 <- suppressWarnings(try(sirspline(
      coh.data = c,
      coh.obs = c('from0to1', 'from0to2'),
      coh.pyrs = 'pyrs',
      subset = year %in% 1990:2008,
    ...
    sp4 <- suppressWarnings(try(sirspline(
      coh.data = c,
      coh.obs = c('from0to1', 'from0to2'),
      coh.pyrs = 'pyrs',
      subset = year %in% 1990:2008,
...
testthat::test_that("print accepts a function and subset works", {
  ...
  testthat::expect_warning(
    pl1 <- sir(
      ...
      ref.rate = 'haz',
      adjust = c('agegroup', 'year', 'sex'),
      print = list(year.int = findInterval(year, c(1989, 2000, 2010))),
      mstate = 'cause'
    )
  )
  testthat::expect_warning(
...
testthat::test_that("sir_ratio", {
  ...
  testthat::expect_equal(
    sir_ratio(x = c(64, 45.6), y = c(25, 23.7), digits = 2)[2:3],
    c(lower = 0.83, upper = 2.21)
  )
  # other tests
  testthat::expect_message(sir_ratio(
    s1,
    s2,
    digits = 2,
    alternative = 'less',
  ...
  testthat::expect_equal(
    sir_ratio(s1, s2),
    sir_ratio(c(120, 150), c(230, 150)),
    tolerance = 0.001
  )
  testthat::expect_equal(
    sir_ratio(s1, c(230, 150)),
    sir_ratio(c(120, 150), s2),
    tolerance = 0.001
  )
  testthat::expect_equal(
    sir_ratio(s1, s2)[1],
    c(sir_ratio = round((120 / 150) / (230 / 150), 3)),
    tolerance = 0.001
  )
})
...
testthat::test_that("sir_exp and sir_ag and sir_lexis are working", {
  ...
  # AGGRE
  testthat::expect_message(
    a1 <- sir_ag(x1, obs = 'from0to1', conf.type = 'profile')
  )
  testthat::expect_message(
    a0 <- sir_ag(x1, obs = from0to1, conf.type = 'profile')
  )
  a0 <- sir_ag(x1, obs = 'from0to1', print = list(period))
  a0 <- sir_ag(x1, obs = 'from0to1', print = list(per = period))
  testthat::expect_message(
    a0 <- sir_ag(x1, obs = from0to1, exp = d.exp, pyrs = pyrs)
  )
  ...
  setkey(a6, period)
  testthat::expect_equal(
    data.table(a5)[order(period, surv.int)],
    data.table(a6)[order(period, surv.int)]
  )
  ...
  BL <- list(
    fot = 0:5,
    per = c('2003-01-01', '2008-01-01', '2013-01-01'),
    age = c(0, 75, 100)
  )
  ...
  e2 <- data.table(e2)
  testthat::expect_equal(
    e1[, .(pyrs, sir, sir.lo, sir.hi, p_value)],
    e2[, .(pyrs, sir, sir.lo, sir.hi, p_value)]
  )
