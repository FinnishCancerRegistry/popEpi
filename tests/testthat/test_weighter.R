...
testthat::test_that("makeWeightsDT works with various weights & adjust spesifications", {
  ...
  adjust <- quote(list(
    agegr = popEpi:::cutLow(dg_age, c(0, seq(15, 85, 5), Inf))
  ))
  ...
  testthat::expect_equal(
    DT[, sum(pyrs), keyby = agegr],
    dt[, sum(pyrs), keyby = agegr],
    check.attributes = FALSE
  )
  testthat::expect_equal(
    DT[, sum(pyrs), keyby = gender],
    dt[, sum(pyrs), keyby = gender],
    check.attributes = FALSE
  )
})
...
testthat::test_that("weighter works with a list of values arguments", {
  ...
  DT1 <- makeWeightsDT(dt, values = vals, print = "sex")
  testthat::expect_equivalent(
    DT1,
    dt[,
      lapply(.SD, sum),
      by = sex,
      .SDcols = c("pyrs", "from0to1", "from0to0")
    ]
  )

  dt$agegr <- cut(dt$dg_age, c(0, 45, 75, Inf))
  DT2 <- makeWeightsDT(dt, values = vals, print = c("sex", "agegr"))
  testthat::expect_equivalent(
    DT2,
    dt[,
      lapply(.SD, sum),
  ...
  testthat::expect_warning(
    {
      DT1 <- makeWeightsDT(
        dt,
        values = list(c("pyrs", "from0to1"), quote(from0to0)),
        adjust = "agegr",
        weights = "internal",
        internal.weights.values = "pyrs",
        NA.text = naTxt
