...
testthat::test_that("cutLowMerge merges succesfully what is intended", {
  ...
  all_names_present(
    popEpi::sire,
    c("sex", "bi_date", "dg_date", "ex_date", "status")
  )
  ...
  sr1[,
    agegroup := popEpi:::cutLow(agegroup, breaks = sort(unique(pm$agegroup)))
  ]
...
testthat::test_that("detectEvents works as intended", {
  ...
  ## really be censoring.
  x[,
    event := detectEvents(x, breaks = attr(x, "breaks")["fot"], by = "lex.id")
  ]
...
testthat::test_that("comp_pp_weighted_figures produces intended results", {
  ...
  testthat::expect_equal(
    x[event %in% 1:2, pp],
    x[event %in% 1:2, from0to0.pp + from0to1.pp]
  )
  testthat::expect_equal(
    x[event %in% 1:2, sum(pp)],
    x[, sum(from0to0.pp + from0to1.pp)]
  )
...
testthat::test_that("evalPopFormula & usePopFormula output is stable", {
  ...
  testthat::expect_equal(
    data.table(r9),
    res[, list(lex.Xst, "factor(sex)" = factor(sex))]
  )
  ...
  testthat::expect_equal(
    lapply(rd, names),
    list(y = "fot", print = NULL, adjust = "factor(sex + 1)", formula = NULL)
  )
