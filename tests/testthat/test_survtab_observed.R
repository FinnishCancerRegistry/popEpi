testthat::context(
  "CIF's & surv.obs's congruence & comparison w/ survival::survfit"
)
...
testthat::test_that("surv.obs about the same as Kaplan-Meier & CIFs close to Aalen-Johansen", {
  ...
  fb <- setdiff(BL$fot, 0)
  su.km <- survival::survfit(
    Surv(time = fot, time2 = fot + lex.dur, event = lex.Xst != "alive") ~ 1,
    data = x,
    id = lex.id
  )
  ...
  su.cif <- survival::survfit(
    Surv(time = fot, time2 = fot + lex.dur, event = lex.Xst) ~ 1,
    data = x,
    id = lex.id
  )
...
testthat::test_that("survtab status argument works as expected", {
  ...
  testthat::expect_error(
    ...
    regexp = paste0(
      "Some status indicators (3648 values in total) were NA. ",
...
testthat::test_that("survtab works with more complicated estimation", {
  ...
  x$stat <- factor(x$status, levels = 0:2, labels = c("alive", "canD", "othD"))
  x$enStat <- factor(
    rep(0L, nrow(x)),
    levels = 0:2,
    labels = c("alive", "canD", "othD")
  ...
  xl <- Lexis(
    entry = list(FUT = 0, AGE = dg_age, CAL = get.yrs(dg_date)),
  ...
  ## observed survival
  st1 <- survtab(
    Surv(time = FUT, event = lex.Xst) ~ factor(sex, 0:1, c("male", "female")) +
      period +
      adjust(agegr),
    data = xl,
  ...
  st2 <- survtab_ag(
    FUT ~ factor(sex, 0:1, c("male", "female")) + period + adjust(agegr),
    weights = list(agegr = as.numeric(table(x$agegr))),
    data = ag,
    surv.type = "surv.obs",
    d = "from0to1"
