...
x <- lexpand(sire, breaks=BL, status = status)
...
sire2 <- copy(sire)
sire2 <- lexpand(sire2, breaks=list(per = c(2008,2013)), status=status)
...
test_that("surv.obs about the same as Kaplan-Meier", {
  expect_equal(st[, surv.obs] ,  su.km[, V1], tolerance = 0.0032, scale=1)
})
test_that("CIFS are close to Aalen-Johansen estimates", {
...
st <- survtab(x, surv.type="cif.obs", by.vars="agegr")
