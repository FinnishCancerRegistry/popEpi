...
surv_brenner_weight_dt <- function(
  ...
  weight_dt <- merge(
    ...
    by = intersect(
      setdiff(names(standard_weight_dt), "weight"),
      setdiff(names(observed_weight_dt), "weight")
    ),
  ...
  if (length(bad_row_nos) > 0) {
    ...
    # @codedoc_comment_block popEpi::surv_brenner_weight_dt
      ...
      stratum_col_nms <- setdiff(
        names(weight_dt),
        c("weight_standard", "weight_observed")
      )
...
surv_individual_weights <- function(
  ...
  # @codedoc_comment_block popEpi::surv_individual_weights
  weight_dt <- surv_brenner_weight_dt(
    standard_weight_dt = standard_weight_dt,
    observed_weight_dt = observed_weight_dt,
